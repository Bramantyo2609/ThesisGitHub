...
void FACTORYIOTEST_init__(FACTORYIOTEST *data__, BOOL retain) {
  __INIT_VAR(data__->I_START,__BOOL_LITERAL(FALSE),retain)
  __INIT_VAR(data__->I_STOP,__BOOL_LITERAL(FALSE),retain)
  __INIT_VAR(data__->Q_CONVEYOR,__BOOL_LITERAL(FALSE),retain)
}

// Code part
void FACTORYIOTEST_body__(FACTORYIOTEST *data__) {
  // Initialise TEMP variables

  __SET_VAR(data__->,Q_CONVEYOR,,(__GET_VAR(data__->I_STOP,) && (__GET_VAR(data__->I_START,) || __GET_VAR(data__->Q_CONVEYOR,))));

  goto __end;

__end:
  return;
} // FACTORYIOTEST_body__() 
